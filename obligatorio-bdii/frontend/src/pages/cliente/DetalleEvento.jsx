    ...
    <Box sx={{ width: '100%', maxWidth: 380, mx: 'auto' }}>
      <svg viewBox="0 0 320 260" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', display: 'block' }}>
        ...
        {SECTOR_PATHS.map(sp => {
          ...
          const isSelected = sectorSeleccionado?.codigo === sp.id;
          ...
          return (
            <g key={sp.id}>
              ...
              >
                {agotado ? `${SECTOR_NOMBRES[sp.id]?.split(' ')[1] || SECTOR_NOMBRES[sp.id]} · Agotado` : `${SECTOR_NOMBRES[sp.id]?.split(' ')[1] || SECTOR_NOMBRES[sp.id]} · USD ${sector.costo}`}
              </text>
            </g>
          );
        })}
